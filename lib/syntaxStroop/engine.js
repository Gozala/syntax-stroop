...
var PR_STRING = exports.PR_STRING = 'str';
/** token style for a keyword */
var PR_KEYWORD = exports.PR_KEYWORD = 'kwd';
/** token style for a comment */
var PR_COMMENT = exports.PR_COMMENT = 'com';
/** token style for a type */
var PR_TYPE = exports.PR_TYPE = 'typ';
/** token style for a literal value.  e.g. 1, null, true. */
var PR_LITERAL = exports.PR_LITERAL = 'lit';
/** token style for a punctuation string. */
var PR_PUNCTUATION = exports.PR_PUNCTUATION = 'pun';
/** token style for a punctuation string. */
var PR_PLAIN = exports.PR_PLAIN = 'pln';

/** token style for an sgml tag. */
var PR_TAG = exports.PR_TAG = 'tag';
/** token style for a markup declaration such as a DOCTYPE. */
var PR_DECLARATION = exports.PR_DECLARATION = 'dec';
/** token style for embedded source. */
var PR_SOURCE = exports.PR_SOURCE = 'src';
/** token style for an sgml attribute name. */
var PR_ATTRIB_NAME = exports.PR_ATTRIB_NAME = 'atn';
/** token style for an sgml attribute value. */
var PR_ATTRIB_VALUE = exports.PR_ATTRIB_VALUE = 'atv';
 ...
 * @returns {Boolean}
 */
var PR_NOCODE = exports.PR_NOCODE = 'nocode';
...
var REGEXP_PRECEDER_PATTERN = function () {
    ...
    for (var i = 0; i < preceders.length; ++i) {
        ...
        if (isWordChar(preceder.charAt(0))) pattern += '|\\b' + preceder;
        else pattern += '|' + preceder.replace(/([^=<>:&])/g, '\\$1');
    }
 ...
 * @param {String} html     text to unescape
...
/**
 * Checks is the given node's innerHTML normally unescaped?
 */
...
/** returns a function that expand tabs to spaces.  This function can be fed
  ...
  * @param {Number}                         Number of spaces in a tab
...
function appendDecorations(offset, sourceCode, langHandler, out) {
    ...
    if (offset)
        for (var i = decorations.length; (i -= 2) >= 0;) decorations[i] += offset;
...
/** Given triples of [style, pattern, context] returns a lexing function,
  ...
  */
var createSimpleLexer = exports.createSimpleLexer = function createSimpleLexer(shortcutStylePatterns, fallthroughStylePatterns) {
    var shortcuts = {};
    (function () {
    ...
    return function (sourceCode, opt_basePos) {
        ...
        while (tail.length) {
            ...
            if (!isEmbedded) decorations.push(opt_basePos + pos, style);
                ...
                appendDecorations(opt_basePos + pos, token.substring(0, embeddedSourceStart), decorate, decorations);
                appendDecorations(opt_basePos + pos + embeddedSourceStart, token.substring(embeddedSourceStart, embeddedSourceEnd), langHandlerRegistry[lang], decorations);
                appendDecorations(opt_basePos + pos + embeddedSourceEnd, token.substring(embeddedSourceEnd), decorate, decorations);
            }
            pos += token.length;
            ...
            if (style === PR_PLAIN) {
                ...
                for (var j = 0, m = subDecs.length; j < m; j += 2) {
                    ...
                    if (subStyle === PR_PLAIN) {
                        ...
                        if (token === '.') subDecs[j + 1] = PR_PUNCTUATION;
                        else if (token in keywords) subDecs[j + 1] = PR_KEYWORD;
    ...
    for (var i = 0; i < decorations.length; i += 2) {
        ...
        if (style === PR_ATTRIB_NAME) {
            ...
            if (nextValueIsSource) {
                ...
                } else spliceArrayInto(attribSourceDecorations, decorations, i, 2);
            }
...
function recombineTagsAndDecorations(sourceText, extractedTags, decorations) {
    ...
    var tabExpander = makeTabExpander(conf.PR_TAB_WIDTH);
    ...
    function emitTextUpTo(sourceIdx) {
        if (sourceIdx > outputIdx) {
            ...
            var htmlChunk = textToHtml(tabExpander(sourceText.substring(outputIdx, sourceIdx)))
                .replace(lastWasSpace ? startOrSpaceRe : adjacentSpaceRe, '$1<span class="space"> </span>');
    ...
    if (openDecoration) html.push('</span>');
    return html.join('');
}
...
/** Register a language handler for the given file extensions.
  ...
  */
var registerLangHandler = exports.registerLangHandler = function registerLangHandler(handler, fileExtensions) {
    for (var i = fileExtensions.length; --i >= 0;) {
        var ext = fileExtensions[i];
        if (!langHandlerRegistry.hasOwnProperty(ext)) langHandlerRegistry[ext] = handler;
        else system.log.info('cannot override language handler %s', ext);
    }
};

registerLangHandler(decorateSource, ['default-code']);
registerLangHandler(decorateMarkup, ['default-markup', 'htm', 'html', 'mxml', 'xhtml', 'xml', 'xsl']);
registerLangHandler(sourceDecorator({'keywords': CPP_KEYWORDS, 'hashComments': true, 'cStyleComments': true}), ['c', 'cc', 'cpp', 'cxx', 'cyc', 'm']);
registerLangHandler(sourceDecorator({'keywords': CSHARP_KEYWORDS, 'hashComments': true, 'cStyleComments': true}), ['cs']);
...
/**
 * Function which takse source and returns hilighted html source
 * @param {String} sourceCode           Source
 * @param {String} language             Optional. Code language
 */
exports.highlight = prettyPrintOne = function prettyPrintOne(sourceCode, opt_langExtension) {

    try {
        sourceCodeHtml = textToHtml(sourceCode);
